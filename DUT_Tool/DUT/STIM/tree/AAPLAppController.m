/*
 ...
 Abstract:
 Application Controller object, and NSOutlineView dataSource/delegate.
  Also demonstrates 10.7 NSOutlineView insertion/deletion/move API.
 */

#import "AAPLAppController.h"
#import "AAPLSimpleNodeData.h"
#import "AAPLImageAndTextCell.h"

@interface AAPLAppController () {
...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL allowOnDropOnContainer;
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL allowOnDropOnLeaf;
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL allowBetweenDrop;

@property (NS_NONATOMIC_IOSONLY, readonly, copy) NSImage *randomIconImage;
...
@implementation AAPLAppController
...
- (IBAction)addContainer:(id)sender {
    ...
    AAPLSimpleNodeData *childNodeData = [[AAPLSimpleNodeData alloc] initWithName:@"New Container"];
...
- (IBAction)sortData:(id)sender {
    ...
    [_rootTreeNode sortWithSortDescriptors:@[sortDescriptor] recursively:YES];
...
- (NSImage *)randomIconImage {
    ...
    NSImage *result = _iconImages[imageNum];
    imageNum++;
...
- (NSTreeNode *)treeNodeFromDictionary:(NSDictionary *)dictionary {
    ...
    NSString *nodeName = dictionary[NAME_KEY];
    AAPLSimpleNodeData *nodeData = [AAPLSimpleNodeData nodeDataWithName:nodeName];
    ...
    for (id item in children) {
        ...
        if ([item isKindOfClass:[NSDictionary class]]) {
            ...
            AAPLSimpleNodeData *childNodeData = [[AAPLSimpleNodeData alloc] initWithName:item];
            childNodeData.container = NO;
