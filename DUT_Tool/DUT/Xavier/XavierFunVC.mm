...
@interface XavierFunVC ()

@end

@implementation XavierFunVC
...
{
//    self = [super init];
    self = [super initWithNibName:@"XavierFunVC" bundle:[NSBundle bundleForClass:[self class]]];
...
{
    XavierUinitVC * c = [[XavierUinitVC alloc] initialwithID:index];
    [arrUnit addObject:c];
...
-(void)SetUpUnit:(NSMutableDictionary *)m_dicConfiguration
{
    for (int i=0; i<[arrUnit count];i++)
    {
        ...
        NSString * key1 = [NSString stringWithFormat:@"%@%d%d",kRequest,i,mID];
        NSString * key2 = [NSString stringWithFormat:@"%@%d%d",kSubscribe,i,mID];
        if (mID > 9) {
        ...
        XavierUinitVC *controller = (XavierUinitVC *)[arrUnit objectAtIndex:i];
